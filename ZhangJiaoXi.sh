...
config_mtp() {
    cd $WORKDIR
    echo -e "检测到配置文件不存在, 准备指引生成!" && print_line
    while true; do
        default_port=443
        echo -e "请输入一个连接端口 [1-65535]"
        ...
        echo -e "[\033[33m错误\033[0m] 请重新输入一个连接端口 [1-65535]"
    done
...
info_mtp() {
    ...
    if [ $? == 1 ]; then
        ...
        echo -e "MTProxy测试版: \033[32m运行中\033[0m"
        echo -e "IP：\033[31m$public_ip\033[0m"
        echo -e "端口：\033[31m$port\033[0m"
        echo -e "Secret:  \033[31m$client_secret\033[0m"
        echo -e "链接: tg://proxy?server=${public_ip}&port=${port}&secret=${client_secret}"
    else
        echo -e "MTProxy(自用)版: \033[33m已停止\033[0m"
    fi
}
...
run_mtp() {
    ...
    status_mtp
    if [ $? == 1 ]; then
        echo -e "提醒：\033[33mMTProxy(自用)版已经运行，请勿重复运行!\033[0m"
    else
